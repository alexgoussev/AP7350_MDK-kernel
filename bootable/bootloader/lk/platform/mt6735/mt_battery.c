...
int hw_charging_get_charger_type(void)
{
    ...
    if(1 == hw_bc11_DCD())
    {
         ...
         {             
             ...
             dprintf(CRITICAL, "step A1 : Apple 2.1A CHARGER!\r\n");
         }
         else
         {
             CHR_Type_num = NONSTANDARD_CHARGER;
             dprintf(CRITICAL, "step A1 : Non STANDARD CHARGER!\r\n");
         }
    }
    else
    {
         ...
         if(1 == hw_bc11_stepA2())
         {
             ...
             if(1 == hw_bc11_stepB2())
             {
                 ...
                 dprintf(CRITICAL, "step B2 : STANDARD CHARGER!\r\n");
             }
             else
             {
                 CHR_Type_num = CHARGING_HOST;
                 dprintf(CRITICAL, "step B2 :  Charging Host!\r\n");
             }
         }
         else
         {
             CHR_Type_num = STANDARD_HOST;
             dprintf(CRITICAL, "step A2 : Standard USB Host!\r\n");
         }
...
void pchr_turn_on_charging(kal_bool bEnable)
{
...
#ifdef MTK_FAN5405_SUPPORT
    fan5405_hw_init();
    fan5405_turn_on_charging(bEnable);
	...
	if(KAL_TRUE == bEnable)
	{
		...
		{
			dprintf(CRITICAL, "[eta6005]lk charging_enable :usb\r\n");
		...
		{
			dprintf(CRITICAL, "[eta6005]lk charging_enable :ac\r\n");
			mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_MODE_GPIO);	
			mt_set_gpio_dir(GPIO_OTG_DRVVBUS_PIN,GPIO_DIR_OUT);
