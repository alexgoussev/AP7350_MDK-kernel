...
void pchr_turn_on_charging(kal_bool bEnable)
{
	...
	if(KAL_TRUE == bEnable)
	{
		...
		{
			print("[eta6005]pl charging_enable :usb\r\n");
		...
		{
			print("[eta6005]pl charging_enable :ac\r\n");
			mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_MODE_GPIO);  
			mt_set_gpio_dir(GPIO_OTG_DRVVBUS_PIN,GPIO_DIR_OUT);
...
#ifdef ETA6005_ENABLE
	chr_type=mt_charger_type_detection();
	print("eta6005 pl chr_type=%d\n",chr_type);
#endif
...
*/
	...
	upmu_set_reg_value(0xA88,0x68); //vsram 1.25v
		...
		{
#if !CFG_EVB_PLATFORM
		//	pl_check_bat_protect_status();//remove by sen.luo,\D2\F2ΪFAN5405\D4\DA\D5\E2\C0\EF\CAǲ\BB\C4ܹ\A4\D7\F7\B5\C4
#endif
		}
