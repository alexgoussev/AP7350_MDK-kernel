...
static const gid_t kGroups[] = { AID_PACKAGE_INFO, AID_MOUNT, AID_INET, AID_NET_RAW};
static const char* m_sdcard_dest_path;
static const char* m_sdcard_source_path;
...
static unsigned long long internal_sdcard_free_size_threshold = 0;
...
static bool get_caller_has_rw_locked(struct fuse* fuse, const struct fuse_in_header *hdr) {
    ...
    if (hashmapContainsKey(fuse->appid_with_rw, (void*) (uintptr_t) appid)) {
       return (bool)hashmapGet(fuse->appid_with_rw, (void*)(uintptr_t)appid);
    }
    else {
        ...
        const struct fuse_in_header* hdr, const struct fuse_mkdir_in* req, const char* name)
{
    ...
    if(!strcmp(fuse->root.name, share_sdcard_source_path) && !strcmp(fuse->root.name, parent_path)){
      ...
      if(i == name_len){
        ...
        /* it means the path is share_sdcard_source_path, it must be uid. So create a symlink in /storage/emulated/<uid> to /mnt/shell/emulated/<uid> for native process */
...
static int handle_users(struct fuse* fuse, struct fuse_handler* handler,
        const struct fuse_in_header* hdr, const char* name)
{
    bool has_rw;
    struct node* parent_node;
    ...
    sprintf(users_path, "%s", m_sdcard_dest_path);
    char* start = strstr(child_path, m_sdcard_source_path);
    if (start != NULL) {
        strcat(users_path, start + strlen(m_sdcard_source_path));       
    }

    LOG("handle_users, parent_path='%s', child_path='%s', name='%s', users_path='%s' \n", parent_path, child_path, name, users_path);

    int status;
    const char *args[4];
    args[0] = "/system/bin/vdc";
    args[1] = "storage";
    args[2] = "kill_users";
    args[3] = users_path;
    android_fork_execvp(ARRAY_SIZE(args), (char **)args, &status, false, true);

    strcat(users_path, " (deleted)");
    args[0] = "/system/bin/vdc";
    args[1] = "storage";
    args[2] = "kill_users";
    args[3] = users_path;
...
#ifdef LIMIT_SDCARD_SIZE
    if (!strncmp(fuse->root.name, share_sdcard_source_path, fuse->root.namelen)) {
        pthread_mutex_lock(&fuse->lock);
...
//    case FUSE_SYMLINK:
    ...
    case FUSE_USERS: {
        const char* name = data;
        return handle_users(fuse, handler, hdr, name);
    }
...
static int read_package_list(struct fuse *fuse) {
    ...
    while (fgets(buf, sizeof(buf), file) != NULL) {
        ...
        if (sscanf(buf, "%s %d %*d %*s %*s %s", package_name, &appid, gids) == 3) {
            ...
            if (is_found == false) {
                if (!hashmapContainsKey(fuse->appid_with_rw, (void*) (uintptr_t) appid)){
                    hashmapPut(fuse->appid_with_rw, (void*) (uintptr_t) appid, (void*) (uintptr_t) 0);
                }
            }
        }
    }
...
static int run(const char* source_path, const char* dest_path, uid_t uid,
    ...
    m_sdcard_dest_path = strdup(dest_path);
    m_sdcard_source_path = strdup(source_path);
    ...
    umask(0);
#if !defined(MTK_2SDCARD_SWAP) && defined(MTK_SHARED_SDCARD)
if(!strcmp(fuse.root.name, share_sdcard_source_path))
{
    DIR *d;
    struct dirent *de;
    struct stat st;
...
static int set_env_value(char * name,char * value,unsigned int value_max_len) {
	...
	if ((data_fd = open(share_sdcard_source_path, O_RDONLY)) < 0) {			
		ERROR("open data fail\n");					
		goto FAIL_RUTURN;		
	}
	if (statfs(share_sdcard_source_path, &stat) < 0) {
        ERROR("get fs status fail\n");
		goto FREE_FD;
	}
	data_free_size = stat.f_bfree * stat.f_bsize;
	close(data_fd);
	if(value_new > data_free_size) {
		ERROR("new value %llx more than data free size %llx, setting fail\n", value_new, data_free_size);
		goto FAIL_RUTURN;
	}
	...
	if ((ret = ioctl(env_fd, ENV_WRITE, &env_ioctl_obj))) {
		ERROR("Set env for %s check fail ret = %d, errno = %d.\n", name,ret, errno);
...
#if defined(MTK_2SDCARD_SWAP) && defined(MTK_SHARED_SDCARD)
    if (!strcmp(source_path, share_sdcard_source_path)) {
        int hasExternalSd = 0;
        int fd_externalsd;
        property_get("vold.path.internal_sd", internal_sd_path, "");
        LOG("vold.path.internal_sd='%s'\n", internal_sd_path);
