...
void LogBuffer::prune(log_id_t id, unsigned long pruneRows, uid_t caller_uid) {
    ...
    if (pruneRows > elements) {
        pruneRows = elements;
        kernel_log_print("logcat -c..fix pruneRows to %lu", pruneRows);
    }
    ...
    if (oldest && (oldest->getSkipAhead() != 0)) {
...
#ifdef HAVE_AEE_FEATURE
                        char aee_msg[255];
                        memset(aee_msg, 0x0, sizeof(aee_msg));
                        snprintf(aee_msg, sizeof(aee_msg) - 1, "Android log too much, reader(pid:%d) has been released!", oldest->getPid());
                        aee_system_warning("logd", NULL, DB_OPT_DEFAULT, aee_msg);
#endif
                        oldest->release_Locked();
                    } else {
                        oldest->triggerSkip_Locked(pruneRows);
                        kernel_log_print("Android log reader(%d) has lost %d logs", oldest->getPid(), pruneRows);
                    }
                }
                break;
            }
...
log_time LogBuffer::flushTo(
    ...
    pthread_mutex_lock(&mLogElementsLock);
    /*find the mLogElements from end {*/
    if (!me->mIsFirstDump)
    {
        it = mLogElements.end();
        it--;
        for (it; it != mLogElements.begin(); --it) {
