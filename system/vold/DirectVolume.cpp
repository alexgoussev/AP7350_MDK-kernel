...
pthread_mutex_t part_rescan_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t part_rescan_cond = PTHREAD_COND_INITIALIZER;
volatile int part_rescan_wait = WAIT_ON_NO_EVENT;
...
bool PathInfo::match(const char *path, bool is_full_match)
{
    switch (patternType) {
    case prefix:
    {
...
DirectVolume::DirectVolume(VolumeManager *vm, const fstab_rec* rec, int flags) :
    ...
    if (strcmp(rec->mount_point, "auto") != 0) {
	   ALOGE("Vold managed volumes have specified mount point '%s'", rec->mount_point);
...
DirectVolume::DirectVolume(VolumeManager *vm, const char *label, const char *mount_point, 
    ...
    snprintf(mount, PATH_MAX, "%s/%s-%s", Volume::MEDIA_DIR, label, description);
    mMountpoint = strdup(mount);
    snprintf(mount, PATH_MAX, "%s/%s/%s-%s", Volume::FUSE_DIR, label, label,description);
    mFuseMountpoint = strdup(mount);
...
int DirectVolume::addPath(const char *path) {
    ...
    char *not_device_tree = strstr(path, "/devices/platform");
    if (not_device_tree) {
        SLOGI("Not support device tree");
        ...
        SLOGI("Support device tree");
        char *p = strstr(path, "MSDC0");
        if (p) {
            SLOGI("This is emmc storage (%s)", path);
            mIsEmmcStorage = true;
        }		
...
bool DirectVolume::isPathUsbOtg(const char *path) {
    ...
    return (strstr(path, get_usb_device_path_keyword()) != NULL);
}
...
int DirectVolume::handleBlockEvent(NetlinkEvent *evt) {
    ...
    for (it = mPaths->begin(); it != mPaths->end(); ++it) {
        ...
        bool isMatch = (*it)->match(dp); 
#ifdef MTK_MULTI_PARTITION_MOUNT_ONLY_SUPPORT
        if (strstr(dp, get_usb_device_path_keyword())) {
            isMatch = (*it)->match(dp, true); 
        }
#endif
                ...
                if (!strcmp(devtype, "disk")) {
                    ...
                    #endif
                    {
                        ...
                        if(part_rescan_wait == WAIT_ON_ADD_EVENT) {
                            SLOGD("pthread_cond_signal, part_rescan_wait=%d", part_rescan_wait);
...
void DirectVolume::handleUsbOtgPartitionAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    SLOGD("usbotg: DirectVolume handleUsbOtgPartitionAdded mDiskMajor = %d", mDiskMajor);
    ...
    if (!mPendingPartCount) {
        ...
        SLOGD("Dv:partAdd: pending mask now = 0x%x", mPendingPartCount);
    }
}
...
void DirectVolume::handleUsbOtgPartitionRemoved(const char *devpath, NetlinkEvent *evt) {
    ...
    */
    state = getState();
    SLOGD("usbotg: DirectVolume handleUsbOtgPartitionRemovedVolume %s %s partition %d:%d removed state:%d\n", getLabel(), getMountpoint(), major, minor, state);

    while( state == Volume::State_Checking)
    {
        SLOGD("usbotg: DirectVolume handleUsbOtgPartitionRemoved removed state:%d\n", state);
        usleep(30000);
        state = getState();
        counter++;
        if( counter == 10)
        {
            SLOGD("usbotg: DirectVolume handleUsbOtgPartitionRemoved timeout removed state:%d counter:%d\n", state, counter);
            break;
        }
    }
    SLOGD("usbotg: DirectVolume handleUsbOtgPartitionRemoved finish removed state:%d counter:%d\n", state, counter);    
...
void DirectVolume::handleDiskChanged(const char * /*devpath*/,
    ...
    if(evt->findParam("DISK_RO")) {
       /* If DISK_RO param is present, this is a notification of mode changing between RO/RW
             ...
             */
       SLOGI("vold doesn't support DISK_RO. Skip!");
       return;
    }
    const char *tmp = evt->findParam("NPARTS");
    if (tmp) {
    ...
    if (getState() != Volume::State_Formatting) {
        if(evt->findParam("DISK_MEDIA_CHANGE")) {     
            SLOGI("disk change with DISK_MEDIA_CHANGE");
            snprintf(nodepath, sizeof(nodepath), "/dev/block/vold/%d:%d", major, minor);
            SLOGI("Check device node (%s) existence in (%s)!", nodepath, __func__);    
        
            if (-1 == (fd = open(nodepath, O_RDONLY, 0644)) ) {
                SLOGI("Node Path (%s) open fail, do unmount, mDiskNumParts=%d!", nodepath, mDiskNumParts);
                unmountVol(true, false);    
            } else {
                SLOGI("Node Path (%s) open success with mDiskNumParts=(%d)!", nodepath, mDiskNumParts);
                close(fd);
            }                
        } else if(part_rescan_wait == WAIT_ON_NO_EVENT){
            if (mDiskNumParts == 0) {  
                SLOGI("SLOGI(possible no partition media inserted, do mount first, mDiskNumParts=%d getState():%d", mDiskNumParts, getState());
                if(getState() == Volume::State_Idle){
                    if(mountVol() != 0) {
                        mRetryMount = true;
                    }
                ...
                SLOGI("possible need next PartitionAdd uevent comes");
                setState(Volume::State_Pending);
            }
        }
    }
...
void DirectVolume::handlePartitionRemoved(const char * /*devpath*/,
     ...
     */
    state = getState();
    SLOGD("Volume %s %s partition %d:%d removed state:%d\n", getLabel(), getMountpoint(), major, minor, state);
    while( state == Volume::State_Checking)
    {
        SLOGD("removed state:%d\n", state);
        usleep(30000);
        state = getState();
        counter++;
     		if( counter == 10)
     		{
     			SLOGD("timeout removed state:%d counter:%d\n", state, counter);
     			break;
     		}
    }
    SLOGD("finish removed state:%d counter:%d\n", state, counter);
    if (state != Volume::State_Mounted && state != Volume::State_Shared) {
        return;
    }
...
int DirectVolume::getDeviceNodes(dev_t *devs, int max) {
    ...
    if (mPartIdx == -1) {
        ...
        int i, j;
        for (i = 0, j=0; i < mDiskNumParts; i++, j++) {
            if (i == max)
                break;
            
            for (; j < MAX_PARTITIONS; j++) {
                if (mPartMinors[j] != -1){
                   break;
                }
            }
            devs[i] = MKDEV(mDiskMajor, mPartMinors[j]);
        }
