...
#define MOUNTPOINT_PREFIX2  "/mnt/media_rw/usbotg-"
...
void NetlinkHandler::handleUsbOtgDiskChanged(NetlinkEvent *evt) {
    char description[255] = {0}, fusemountPoint[255] = {0}, mountPoint[255] = {0},nodePath[255] = {0};
    ...
    if (false == getDescription(devpath, description)) {
    ...
    if (evt->findParam("DISK_EVENT_MEDIA_DISAPPEAR")) {
        ...
        for (i = 0; i < partN; i++) {
            sprintf(fusemountPoint, "%s%s%d", MOUNTPOINT_PREFIX, description, i+1);
            SLOGD("usbotg: NetlinkHandler handleUsbOtgDiskChanged partN = %d, fusemountPoint = %s", partN, fusemountPoint);
            sprintf(mountPoint, "%s%s%d", MOUNTPOINT_PREFIX2, description, i+1);
            ...
            if (0 == UnmountPath(fusemountPoint, true)) {
                ...
                snprintf(cmd, sizeof(cmd), "/system/bin/sh -c \"rm -r %s\"", fusemountPoint);
