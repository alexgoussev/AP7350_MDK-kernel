            ...
            while(token != NULL)
            {
                ...
                if( i==8 )
                {
                    ...
                    if( substr != NULL)
                    {
                       ...
                       if (strstr(token, Volume::FUSE_DIR))
                       {
...
void Process::killProcessesWithOpenFiles(const char *path, int action, const char* need_to_wait_proc_name, bool* need_to_wait) {
    DIR*    dir;
    struct dirent* de;
    ...
    if (need_to_wait_proc_name && need_to_wait) {
      *need_to_wait = false;
    }
    ...
    while ((de = readdir(dir))) {
        ...
        if (need_to_wait_proc_name && !strstr(name, need_to_wait_proc_name)){
            continue;
        }
        ...
        if(!strcmp(name, "/sbin/adbd"))     {
            ...
            if (fd >= 0) {
              ...
              SLOGI("Fail to open %s", USB_DEVICE_PATH);
            }
        }
