...
void copy_log_in_data_tmpfs(char* crypto_blkdev){
    ...
    sprintf(tmp_mount_point, "%s/tmp_mnt", DATA_MNT_POINT);
    sprintf(data_tmpfs_log_path, "%s/data_tmpfs_log", tmp_mount_point);
    ...
    if (fs_mgr_do_mount(fstab, DATA_MNT_POINT, crypto_blkdev, tmp_mount_point)) {
      ...
      sprintf(cmd, "rm -rf %s/*", data_tmpfs_log_path); 
      system(cmd);

      sprintf(cmd, "cp -rp %s/log_temp %s", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      sprintf(cmd, "%s/misc", data_tmpfs_log_path);
      mkdir(cmd, 0775);
      chown(cmd, AID_SYSTEM, AID_MISC);
      sprintf(cmd, "cp -rp %s/misc/mblog %s/misc", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      sprintf(cmd, "%s/mdlog", data_tmpfs_log_path);
      mkdir(cmd, 0775);
      chown(cmd, AID_SHELL, AID_SYSTEM);


      sprintf(cmd, "cp -rp %s/mdlog/bootupLog %s/mdlog", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      sprintf(cmd, "cp -rp %s/aee_exp %s", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);
