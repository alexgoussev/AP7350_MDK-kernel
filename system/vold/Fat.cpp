...
int readboot(	int dosfs, struct bootblock *boot)
{
	...
	if (boot->flags & FAT32) {
		...
		if ((n = lseek(dosfs, boot->FSInfo * boot->BytesPerSec, SEEK_SET)) == -1
		    || (unsigned int)n != boot->FSInfo * boot->BytesPerSec
		...
		if ((n = lseek(dosfs, boot->Backup * boot->BytesPerSec, SEEK_SET)) == -1
		    || (unsigned int)n != boot->Backup * boot->BytesPerSec
	...
	if (boot->flags&FAT32)
		...
		SLOGE("Filesystem too big (%lu clusters) for non-FAT32 partition\n",
		       boot->NumClusters);
		return -1;
	}
	...
	if (boot->NumFatEntries < boot->NumClusters) {
		SLOGE("FAT size too small, %lu entries won't fit into %u sectors\n",
	...
	SLOGI("\n RootCl (Start of Root Directory) : 0x%lx ", boot->RootCl);
	SLOGI("\n FSFree (Number of free clusters acc. FSInfo) : 0x%lx ", boot->FSFree);
	SLOGI("\n FSNext (Next free cluster acc. FSInfo) : 0x%lx ", boot->FSNext);
	SLOGI("\n------------------------------------------------------------");
	SLOGI("\n ClustMask (FAT12:fff, FAT16:ffff, FAT32:fffffff) : 0x%lx ", boot->ClustMask);
	SLOGI("\n NumClusters (cluster numbers) : %lu ", boot->NumClusters);
