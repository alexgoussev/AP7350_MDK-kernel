...
{
   return (strstr(dev_path, get_usb_device_path_keyword()) != NULL) ;
}
...
#ifdef NETLINK_DEBUG
#ifndef MTK_MULTI_PARTITION_MOUNT_ONLY_SUPPORT            
...
#ifdef MTK_2SDCARD_SWAP
    if(bSdCardSwapBootComplete && hit &&  !just_update_status){
...
int VolumeManager::createAsec(const char *id, unsigned int numSectors, const char *fstype,
    ...
    if (usingExt4) {
       ...
       numImgSectors += 2048;
       SLOGI("For 3.10, add 0.02 for ext4 reserve cluseter");
       numImgSectors += numImgSectors/50;
    }
    SLOGI("numImgSectors(%d)", numImgSectors);
...
int VolumeManager::waitForAfCleanupAsec(Volume *v) {
    ...
    if (providesAsec) {
        ...
        SLOGD("Secondary sd card, skip waitForAfCleanupAsec()");
        return 0;    
    }         
...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    int flags = v->getFlags();
    bool primaryStorage = (flags & VOL_PROVIDES_ASEC) != 0;
...
int VolumeManager::unmountVolume(const char *label, bool force, bool revert) {
    ...
    #ifdef MTK_SHARED_SDCARD
	if (v->IsEmmcStorage()) {
	   SLOGI("Volume %s is shared internal sd. Skip umount.", v->getLabel());
	   errno = EACCES;
	   return -1;
	} 	
    #endif
...
int VolumeManager::cleanupAsec(Volume *v, bool force) {
    ...
    if (providesAsec) {
        ...
        SLOGD("Secondary sd card, skip cleanupAsec()");
        return 0;    
    }
...
int VolumeManager::reinitExternalSD(){
	...
	result = ioctl(inode, MSDC_CD_PIN_EN_SDCARD, NULL);

	if(result < 0 ){
             SLOGE("MSDC_CD_PIN_EN_SDCARD ioctl error");
        }
        else { 
	   if(result == 1){  // the ioctrl returns success and support card detection pin 
	     SLOGI("The external sd has detection pin. Skip MSDC_REINIT_SDCARD");
	     close(inode);
	     return 0;
	   }
        }
...
void VolumeManager::swap2Sdcard() {
    ...
    for (i = 0, it = mVolumes->begin(); it != mVolumes->end(); ++it, i++) {
         ...
         if (((*it)->getState() == Volume::State_Idle) && (strstr((*it)->getLabel(), "sdcard"))) {
             numVolume_onBoot++;
         }
    }
...
void VolumeManager::setStoragePathProperty() {
    ...
    char *p;
    const char* one_stoarge_label;
    char one_stoarge_path[PROPERTY_VALUE_MAX];
    ...
    if (numVolume_sdcard == 1) {
       snprintf(one_stoarge_path, sizeof(one_stoarge_path), "%s/%s", Volume::FUSE_DIR, one_stoarge_label);
       if (has_internal_sdcard) {
           property_set("vold.path.internal_sd", one_stoarge_path);         
           create_link_for_mtklogger(one_stoarge_path);
       }
       else {
           property_set("vold.path.external_sd", one_stoarge_path);         
           create_link_for_mtklogger(one_stoarge_path);
       }
    }
    else if (numVolume_sdcard >= 2){
        property_set("vold.path.internal_sd", "/storage/sdcard0");
        property_set("vold.path.external_sd", "/storage/sdcard1");   
        create_link_for_mtklogger("/storage/sdcard1");
    }
}

void VolumeManager::updateProvideAsecFlag() {
    const char* externalStorage = getenv("EXTERNAL_STORAGE");
    VolumeCollection::iterator it;

    #ifdef MTK_SHARED_SDCARD
    if(!strcmp("/storage/emulated/legacy", externalStorage)) {
       externalStorage = "/storage/sdcard0";
    }       
    #endif

    for (it = mVolumes->begin(); it != mVolumes->end(); ++it) {
       if (externalStorage && !strcmp((*it)->getFuseMountpoint(), externalStorage)) {
           SLOGI("The volume(%s) fuse mount point(%s) is same as EXTERNAL_STORAGE. Set VOL_PROVIDES_ASEC to the volume", (*it)->getLabel(), (*it)->getFuseMountpoint());
           (*it)->setFlags((*it)->getFlags()|VOL_PROVIDES_ASEC);
       }
       else {
           (*it)->setFlags((*it)->getFlags()&~VOL_PROVIDES_ASEC);
       }
 ...
 * MTK_2SDCARD_SWAP
 * ********************************************************/
bool VolumeManager::is2SdcardSwapped() {
#ifdef MTK_2SDCARD_SWAP
    return mIs2sdSwapped;
#endif
    return 0;
}
void VolumeManager::setNeedSwapAfterUnmount(bool value) { 
#ifdef MTK_2SDCARD_SWAP
    bNeedSwapAfterUnmount = value ;
#else
    (void*)value;
#endif
}
bool VolumeManager::getNeedSwapAfterUnmount(void) {
#ifdef MTK_2SDCARD_SWAP
    return bNeedSwapAfterUnmount;
#endif
    return 0;
}
void VolumeManager::setNeedSwapAfterMount(bool value) { 
#ifdef MTK_2SDCARD_SWAP
    bNeedSwapAfterMount = value ;
#else
    (void*)value;
#endif
}
bool VolumeManager::getNeedSwapAfterMount(void) {
#ifdef MTK_2SDCARD_SWAP
    return bNeedSwapAfterMount;
#endif
    return 0;
}
void VolumeManager::set2SdcardSwapped(bool isSwapped) {       
#ifdef MTK_2SDCARD_SWAP
    char int_path[PROPERTY_VALUE_MAX] ;
    char ext_path[PROPERTY_VALUE_MAX] ;
    char swap_state_str[2];

    property_get("vold.path.internal_sd", int_path, ""); 
    property_get("vold.path.external_sd", ext_path, ""); 
    ...
    property_set("vold.swap.state", swap_state_str);

    if(mIs2sdSwapped) {
        property_set("vold.path.internal_sd", "/storage/sdcard1");
        property_set("vold.path.external_sd", "/storage/sdcard0");
    }
    else {
        property_set("vold.path.internal_sd", "/storage/sdcard0");
        property_set("vold.path.external_sd", "/storage/sdcard1");
    ...
    if(mIs2sdSwapped) 
        create_link_for_mtklogger("/storage/sdcard0");
    else
        create_link_for_mtklogger("/storage/sdcard1");
