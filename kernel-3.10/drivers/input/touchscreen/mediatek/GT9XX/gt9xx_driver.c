...
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt9xx", 0x5d)};
...
int i2c_write_bytes_non_dma(struct i2c_client *client, u16 addr, u8 *txbuf, int len)
{
    ...
    GTP_INFO("i2c_write_bytes to device %02X address %04X len %d\n", client->addr, addr, len);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    msleep(ms);
    GTP_INFO("WWM GTP RESET! client->addr:%x \n",client->addr);
