...
static inline void cyttsp5_disable_irq(struct cyttsp5_core_data *cd, bool sync)
{
#ifdef CYTTSP5_IRQ_NO_GPIO
	cyttsp5_mtk_gpio_interrupt_disable();/*realize the cyttsp5 TP move after suspend*/
#else
	if (sync)
		disable_irq(cd->irq);
	else
...
static inline void cyttsp5_enable_irq(struct cyttsp5_core_data *cd)
{
#ifdef CYTTSP5_IRQ_NO_GPIO
	cyttsp5_mtk_gpio_interrupt_enable(); /*realize the cyttsp5 TP move after suspend*/
#else
	enable_irq(cd->irq);
#endif
}

static inline void cyttsp5_free_irq(struct cyttsp5_core_data *cd)
{
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (hid_output->command_code == HID_OUTPUT_BL_LAUNCH_APP) {
		cyttsp5_disable_irq(cd, true);
		msleep(20);
		cyttsp5_enable_irq(cd);
	}
...
static irqreturn_t cyttsp5_irq(int irq, void *handle)
{
	...
	printk("zg enter cyttsp5_irq!\r\n");
	rc = cyttsp5_read_input(cd);
	if (!rc)
	{
		printk("zg enter cyttsp5_parse_input!\r\n");
...
#endif
    //RQIU
	printk(KERN_ERR"[cyttsp5] pdata = %p\n",cd->pdata);
	printk(KERN_ERR"[cyttsp5] cpdata = %p\n",cd->cpdata);
	printk(KERN_ERR"[cyttsp5] xres = %p\n",cd->cpdata->xres);
	printk(KERN_ERR"[cyttsp5] init = %p\n",cd->cpdata->init);
	printk(KERN_ERR"[cyttsp5] detect = %p\n",cd->cpdata->detect);
	printk(KERN_ERR"[cyttsp5] hid_desc_register = %d\n",(int)(cd->cpdata->hid_desc_register));
...
#ifdef CYTTSP5_NO_PM_RUNTIME
	cyttsp5_core_rt_resume(dev);
#else
...
static int cyttsp5_event_flag;
static struct task_struct *cyttsp5_event_thread;

void eint_interrupt_handler(void)
{
	cyttsp5_event_flag = 1;
	wake_up_interruptible(&waiter);
}

static int cyttsp5_event_handler(void *unused)
{
	int signal;
	struct cyttsp5_core_data *cd = (struct cyttsp5_core_data *)unused;
	...
	do {
		...
		signal = wait_event_interruptible(waiter,
				cyttsp5_event_flag != 0);
		cyttsp5_event_flag = 0;
...
static int cyttsp5_setup_irq_no_gpio(struct cyttsp5_core_data *cd)
{
	cyttsp5_mtk_gpio_interrupt_register();
	return 0;
}
#else
...
static int cyttsp5_setup_irq(struct cyttsp5_core_data *cd)
{
#ifdef CYTTSP5_IRQ_NO_GPIO
	return cyttsp5_setup_irq_no_gpio(cd);
#else
	return cyttsp5_setup_irq_gpio(cd);
#endif
}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	struct cyttsp5_platform_data *pdata = &_cyttsp5_platform_data;
	...
	dev_err(dev, "%s: platform data ok\n", __func__);
	...
	dev_err(dev, "%s: platform data function ok \n", __func__);
	//note:The driver of CTP should excute powering and GPIO setting firstly,or, 
	//lots of init will occur error.
	//hanguangce 20150814
	...
	msleep(10);
	printk(KERN_ERR"[cyttsp5] %s,%d,GPIO is setted\n",__func__,__LINE__);
	...
	dev_err(dev, "%s: cd   kzalloc ok \n", __func__);
	printk(KERN_ERR"[cyttsp5]cd size = %ld",sizeof(*cd));
...
#endif
	dev_err(dev, "%s: cd->I2CDMABuf_va  dma_alloc_coherent ok \n", __func__);
...
#ifdef CYTTSP5_IRQ_NO_GPIO
	cyttsp5_event_thread = kthread_run(cyttsp5_event_handler, cd,
			"cyttsp5_event_handler");
	if (IS_ERR(cyttsp5_event_thread)) {
		rc = PTR_ERR(cyttsp5_event_thread);		
		goto error_kthread;
	}
#endif
	...
	if (cd->cpdata->detect) {
		...
		printk(KERN_ERR"[cyttsp5] %s,%d,r=%d\n",__func__,__LINE__,rc);
		if (rc) {
			dev_err(cd->dev, "%s: No HW detected\n", __func__);
			rc = -ENODEV;
			goto error_detect;
		}
	}
	...
	setup_timer(&cd->watchdog_timer, cyttsp5_watchdog_timer,
			(unsigned long)cd);
	printk(KERN_ERR"[cyttsp5] %s,%d\n",__func__,__LINE__);
	rc = cyttsp5_setup_irq(cd);
	if (rc < 0) {
		dev_err(dev, "%s: Error, could not setup IRQ\n", __func__);
		goto error_setup_irq;
	}

	dev_err(dev, "%s: add sysfs interfaces\n", __func__);
	 ...
	 */
	dev_err(dev, "%s: call startup\n", __func__);
	rc = cyttsp5_startup(cd, false);

#ifdef CYTTSP5_NO_PM_RUNTIME
	cyttsp5_core_rt_suspend(dev);
#else
