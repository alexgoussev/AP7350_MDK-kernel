...
#endif 
//boway add,m_li,for tp key report,20141025,begin
#if TPD_HAVE_BUTTON
struct touch_vitual_key_map_t
{
   int point_x;
...
static struct touch_vitual_key_map_t touch_key_point_maping_array[]=GTP_KEY_MAP_ARRAY;
#endif
//boway add,m_li,for tp key report,20141025,end
...
//boway add ,m_li,20150106,for phone cover,begin
#if GTP_COVER_SWITCH
static int Switch_ON_OFF = -1;
static struct kobject *gt9xx_tpd_kobj;
static u8 gtp_cover_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
    = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
#endif
//boway add ,m_li,20150106,for phone cover,end
...
//boway add ,m_li,20150106,for phone cover,begin
#if GTP_COVER_SWITCH
static int gtp_send_cover_cfg(struct i2c_client *client)
{
	s32 ret = 1;
#if GTP_DRIVER_SEND_CFG
    s32 retry = 0;
    ...
    GTP_INFO("Driver Send Config");
    for (retry = 0; retry < 5; retry++) {
        ret = gtp_i2c_write(client, gtp_cover_config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        if (ret > 0) {
            break;
        }
    }
...
//boway add ,m_li,20150106,for phone cover,begin
#if GTP_COVER_SWITCH
		const u8 cfg_cover[] = GTP_CFG_COVER;
		u8 cfg_lens_cover = CFG_GROUP_LEN(cfg_cover);
#endif
//boway add ,m_li,20150106,for phone cover,end
...
//boway add ,m_li,20150106,for phone cover,begin
	...
	memset(&gtp_cover_config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
	if (cfg_lens_cover == cfg_len) 
		memcpy(&gtp_cover_config[GTP_ADDR_LENGTH], &cfg_cover, cfg_len);

#if GTP_CUSTOM_CFG
	gtp_cover_config[RESOLUTION_LOC] = (u8) GTP_MAX_WIDTH;
	gtp_cover_config[RESOLUTION_LOC + 1] = (u8) (GTP_MAX_WIDTH >> 8);
	gtp_cover_config[RESOLUTION_LOC + 2] = (u8) GTP_MAX_HEIGHT;
	gtp_cover_config[RESOLUTION_LOC + 3] = (u8) (GTP_MAX_HEIGHT >> 8);

	if (GTP_INT_TRIGGER == 0) 	/* RISING  */
		gtp_cover_config[TRIGGER_LOC] &= 0xfe;
	else if (GTP_INT_TRIGGER == 1) /* FALLING */
		gtp_cover_config[TRIGGER_LOC] |= 0x01;
#endif /* END GTP_CUSTOM_CFG */
	if (cfg_lens_cover != cfg_len) 
		memset(&gtp_cover_config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);	
	...
	for (i = GTP_ADDR_LENGTH; i < cfg_len; i++)
	{
		check_sum += gtp_cover_config[i];
	}
	gtp_cover_config[cfg_len] = (~check_sum) + 1;
...
//boway add ,m_li,20150106,for phone cover,end
...
#endif
	{
		...
		if (flash_cfg_version < 90 && flash_cfg_version > drv_cfg_version) {
			...
			config[cfg_len] = (~check_sum) + 1;
			msleep(50);
			ret = gtp_send_cfg(client);	//boway add for: goodix apk read config version , 20150127
		}
	}
    #endif
...
//boway add ,cai.zhong,20150106,for phone cover,begin
#if GTP_COVER_SWITCH
static int gtp_cover_switch(s32 dir_update)
{
    int ret = 0;
	u8 check_sum = 0;
	int i;
    ...
    if (dir_update)
    {
				if (gtp_send_cover_cfg(i2c_client_point) < 0) {
					ret = -1;
					GTP_ERROR("Send cover config failed.");
				} else {
					ret = 0;
					GTP_DEBUG("Send cover config.");
				}
    }
    else            // charger plugged out
    {
			#if 1 
				config[41] = config[41]|0x80; 
				//config[36] = config[36]|0x80;
				check_sum = 0; 
				for (i = GTP_ADDR_LENGTH; i < cfg_len; i++) { 
				check_sum += config[i];	
				} 
...
#endif
//boway add ,cai.zhong,20150106,for phone cover,end
...
//boway add ,m_li,20150106,for gesture up,begin
...
//boway modify,for TP key report.20141025
...
static int tpd_local_init(void)
{
	...
	GTP_INFO("[succ] Allocate DMA I2C Buffer succ!\n");
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	//GTP_INFO("GTP gtp_hotknot_enabled.");
...
//boway add ,cai.zhong,20150106,for phone cover,begin
static ssize_t gt9xx_tpd_switch_show(struct kobject *kobj,struct kobj_attribute *attr, char *buf)
{
	int len;

	len = sprintf(buf, "%d", Switch_ON_OFF);

	return len;
}

static ssize_t gt9xx_tpd_switch_store(struct kobject *kobj,
				...
				const char * buf, size_t n)
{
	...
	if(1 == on_off)
		err = gtp_cover_switch(1);
	else if (0 == on_off)
		err = gtp_cover_switch(0);
	else 
	{
		err = -1;	
		GTP_ERROR("GTP write failed!!!");
	}

	if(err < 0)
		Switch_ON_OFF = err;
	else
		Switch_ON_OFF = on_off;
	GTP_ERROR("Switch_ON_OFF = %d\n",Switch_ON_OFF);

	return n;
}


static struct kobj_attribute gt9xx_tpd_switch_attr = {
    .attr = {
        .name = "gt9xx_tpd_switch",
        .mode = 0777,
    },
    .show =  &gt9xx_tpd_switch_show,
    .store = &gt9xx_tpd_switch_store,
};


static struct attribute *gt9xx_tpd_attrs[] = {
    &gt9xx_tpd_switch_attr.attr,
    NULL
};


static struct attribute_group gt9xx_tpd_attr_group = {
    .attrs = gt9xx_tpd_attrs,
};
...
static ssize_t show_chipinfo(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "IC: GT%s FW VER: %04x CFG: %d ID: %d\n", pid, vid, cfg_version,vendor_id);
}
...
#if 0
	//boway add ,cai.zhong,20150106,for phone cover,begin
	gt9xx_tpd_kobj = kobject_create_and_add("boway_tpd", NULL);
	if (gt9xx_tpd_kobj == NULL)
		GTP_ERROR("%s: subsystem_register failed\n", __func__);

	if (sysfs_create_group(gt9xx_tpd_kobj, &gt9xx_tpd_attr_group))
		GTP_ERROR("%s:sysfs_create_group failed\n", __func__);
	//boway add ,cai.zhong,20150106,for phone cover,end
#endif
    return 0;
}
