...
extern int mtkts_bts_get_hw_temp(void);//add by sen.luo
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	battery_log(BAT_LOG_FULL,"luosen1 set CV(%d) !!\n\r",cv_voltage);
...
void select_charging_curret(void)
{
	...
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			#if 1
			//add by sen.luo 20150910
			hw_temp = mtkts_bts_get_hw_temp();
			//printk("luosen 003 hw_temp=%d\n",hw_temp);
                  if(hw_temp>=55000)
		{
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
            ...
            battery_log(BAT_LOG_CRTI, "[BATTERY]old temperature = %d, [%d,%d]\r\n", 
		  ...
		  battery_log(BAT_LOG_FULL,"luosen2 set CV(%d) !!\n\r",cv_voltage);
                battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE,&cv_voltage);

                battery_log(BAT_LOG_CRTI, "[BATTERY] charging current is set 350mA, CKT low temperature(0-10) protect !\r\n");
		    }
		    else
		    {
                #ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
			...
			battery_log(BAT_LOG_FULL,"luosen3 set CV(%d) !!\n\r",cv_voltage);
		        battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE,&cv_voltage);
			    #endif
		    }            
            #else
			...
			cv_voltage = BATTERY_VOLT_04_200000_V;
#endif
                     battery_log(BAT_LOG_FULL,"luosen4 set CV(%d) !!\n\r",cv_voltage);
			battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_voltage);
#endif
            #endif
		}
