...
struct epl_sensor_priv
{
    ...
    /*data*/
    u16		    lux_per_count;
    ulong       enable;         	/*record HAL enable status*/
...
static bool read_factory_calibration(void)
{
    ...
    if(epl_sensor.ps.factory.calibration_enable && !epl_sensor.ps.factory.calibrated)
    {
        ...
        else
        {
            ...
            sscanf(buffer, "%d,%d,%d", (int *)&epl_sensor.ps.factory.cancelation, (int *)&epl_sensor.ps.factory.high_threshold, (int *)&epl_sensor.ps.factory.low_threshold);
            set_fs(fs);
    ...
    if(epl_sensor.als.factory.calibration_enable && !epl_sensor.als.factory.calibrated)
    {
        ...
        else
        {
            ...
            sscanf(buffer, "%d", (int *)&epl_sensor.als.factory.lux_per_count);
            set_fs(fs);
        }
...
int epl_sensor_read_als(struct i2c_client *client)
{
...
#endif
    {
        ...
        //filter some tiny values when full covering the alsps hole.
		if(epl_sensor.als.data.channels[1] <= 40)
...
{
    sscanf(buf, "%d",(int *)&epl_sensor.als.factory.lux_per_count);
    return count;
}
...
static int epl259x_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
	u8 databuf[10];    
	...
	sprintf(buf, "EPL259X Chip");
	return 0;
}

static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	struct i2c_client *client = epl_sensor_i2c_client;
	char strbuf[256];
	if(NULL == client)
	{
	...
	epl259x_ReadChipInfo(client, strbuf, 256);
