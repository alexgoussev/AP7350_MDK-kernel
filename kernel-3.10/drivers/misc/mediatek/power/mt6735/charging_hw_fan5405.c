...
static void hw_bc11_done(void)
{
 ...
 static kal_uint32 charging_hw_init(void *data)
 {
	...
	fan5405_reg_config_interface(0x06,0x7f); // ISAFE = 1450mA, VSAFE = 4.44V
	#else
	fan5405_reg_config_interface(0x06,0x70);
	#endif
	#if defined(CONFIG_CKT_NOVA03A_BATTERY)
       fan5405_reg_config_interface(0x02,0xae); //4.36V
	#else
	fan5405_reg_config_interface(0x02,0xba); //4.42V
	#endif
	...
	if ( !charging_init_flag ) {   
		fan5405_reg_config_interface(0x04,0x72); //146mA
...
battery_log(BAT_LOG_FULL, "luosen charging_enable=%d,status=%d\n", enable, status);		
	return status;
 }
...
battery_log(BAT_LOG_FULL, "luosen charging_set_cv_voltage=%d,status=%d\n", register_value, *(kal_uint32 *)(data));		
	return status;
 } 	
 ...
 static kal_uint32 charging_get_current(void *data)
 {
    ...
    *(kal_uint32 *)data = charging_value_to_parameter(CS_VTH,array_size,reg_value);
	battery_log(BAT_LOG_FULL, "luosen charging_get_current=%d,reg_value=%d\n", *(kal_uint32 *)data, reg_value);		
    return status;
 }  
 ...
 static kal_uint32 charging_set_current(void *data)
 {
	...
	battery_log(BAT_LOG_FULL, "luosen charging_set_current=%d,register_value=%d\n", set_chr_current, register_value);		
	return status;
 } 	
...
battery_log(BAT_LOG_FULL, "luosen charging_set_input_current=%d,register_value=%d\n", set_chr_current, register_value);		
	return status;
 } 	
 ...
 static kal_uint32 charging_get_charging_status(void *data)
 {
	...
	battery_log(BAT_LOG_FULL, "luosen charging_get_charging_status=%d,ret_val=%d\n", *(kal_uint32 *)data , ret_val);		
	return status;
 } 	
 ...
 battery_log(BAT_LOG_FULL, "luosen charging_set_hv_threshold=%d,ret_val=%d\n", set_hv_voltage , register_value);		
	 return status;
  }
...
battery_log(BAT_LOG_FULL, "luosen charging_get_hv_status=%d\n",  *(kal_bool*)(data));		
	   return status;
  }
...
battery_log(BAT_LOG_FULL, "luosen charging_get_battery_status=%d\n", *(kal_bool*)(data));	   
	   return status;
 }
