...
static kal_uint32 charging_enable(void *data)
{
    ...
    CHARGER_TYPE chr_type=0;

    if(KAL_TRUE == enable)
    {
    	#ifdef CONFIG_ETA6005_ENABLE
		...
		{
			battery_xlog_printk(BAT_LOG_CRTI, "[eta6005]charging_enable :usb\r\n");
		...
		{
			battery_xlog_printk(BAT_LOG_CRTI, "[eta6005]charging_enable :ac\r\n");
			mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_MODE_GPIO);  
			mt_set_gpio_dir(GPIO_OTG_DRVVBUS_PIN,GPIO_DIR_OUT);
