 ...
 * Filename:
 * ---------
 *	 OV8858qtmipiraw_sensor.c
 *
 * Project:
 * --------
 *	 ALPS MT6752 2lane ov8858r2a
 *
 * Description:
 * ------------
...
#include "ov8858qtmipiraw_Sensor.h"

#define OV8858MT6752LOG
#if define OV8858MT6752LOG
#define SENSORDB LOG_INF2
#elif OV8858MT6580LOG
...
#define LOG_1 SENSORDB("OV88582LANE,MIPI CAM\n")
...
//add by  hhk 2015-6-25
#define OV8858R2AOTP
#define OTP_MID_Qtech               0x06 //ÇðîÑ
#define OTP_MID_LiteArray               0x04 //¹âÕó
#define OTP_MID_Chicony               0x16 //Èº¹â
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8858QT_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.framelength = (2488+200),			//record different mode's framelength
...
{
	SENSORDB("dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	SENSORDB("Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
{
	SENSORDB("le:0x%x, se:0x%x, gain:0x%x\n",le,se,gain);
...
#ifdef OV8858R2AOTP
	write_cmos_sensor( 0x5002, 0x00);//bit[3]=0 OTP_DPC function disable
	mdelay(2);
#endif
...
write_cmos_sensor(0x380f,0xb8);//dc     
write_cmos_sensor(0x3814,0x03);         
write_cmos_sensor(0x3821,0x61);//67       
write_cmos_sensor(0x382a,0x03);         
write_cmos_sensor(0x382b,0x01);         
write_cmos_sensor(0x3830,0x08);         
write_cmos_sensor(0x3836,0x02);         
...
{
	SENSORDB("E! currefps:%d\n",currefps);
  capture_setting(currefps);
}
static void hs_video_setting()
{
	SENSORDB("ERROR! %s NOT READY\n",__FUNCTION__);
}
...
{//Qtech RG_Ratio_Typical 146  BG_Ratio_Typical 12a
	int RG_Ratio_Typical = 0x146, BG_Ratio_Typical = 0x12A;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	kal_uint8 retry = 2;
        ...
        printk("lifeng ov8850 get_imgsensor_id\n");
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				SENSORDB("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858version);
                               printk("lifeng111 ov8850 get_imgsensor_id\n");
				goto CheckSubid;
				}
				else if((read_cmos_sensor(0x302A)) == 0XB1){//we use r2a  
 				ov8858version = OV8858R1A;
				SENSORDB("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858version);
printk("lifeng222 ov8850 get_imgsensor_id\n");	
				return ERROR_SENSOR_CONNECT_FAIL;
				}
				else{
				SENSORDB("read ov8858 R1A R2A bate fail\n");	
printk("lifeng333 ov8850 get_imgsensor_id\n");  
				return ERROR_SENSOR_CONNECT_FAIL;
				}
			}	
			SENSORDB("Read sensor id fail, id: 0x%x,sensor_id=0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
//read otp
  ...
  write_cmos_sensor(0x5002, 0x00);//bit[3]=0 OTP_DPC function disable
  mdelay(2);
  printk("GET sensor moudle id ,Read the sensor OTP\n");
  read_otp(&current_otp);
  //apply_otp(&current_otp);
  //judge OTP_MID_Qtech ID
  printk("OTP_MID=%d\n", current_otp.module_integrator_id);
  if(current_otp.module_integrator_id==OTP_MID_Qtech)
  	{
  	    printk("OTP_MID=OTP_MID_Qtech\n");
		*sensor_id=OV8858QT_SENSOR_ID;
		return ERROR_NONE;
  	}
  else
  	{
  	  printk("OTP_MID is not OTP_MID_Qtech\n");
	  *sensor_id = 0xFFFFFFFF;
	  return ERROR_SENSOR_CONNECT_FAIL;
  	}
#endif  
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+2;
			if (sensor_id == imgsensor_info.sensor_id) {				
				SENSORDB("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			SENSORDB("Read sensor id fail, id: 0x%x,sensor_id =0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
	...
	mdelay(30);
	#ifdef OV8858R2AOTP
		SENSORDB("Apply the sensor OTP\n");
		read_otp(&current_otp);
		apply_otp(&current_otp);
		mdelay(2);
		write_cmos_sensor( 0x5002, 0x08);//bit[3]=1 OTP_DPC function enable
	    mdelay(2);
	#endif
	spin_lock(&imgsensor_drv_lock);
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			SENSORDB("Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n",imgsensor.current_fps/10,imgsensor_info.cap1.max_framerate/10);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate) 
{
	...
	switch (scenario_id) {
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			set_dummy();	
			SENSORDB("error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}	
	return ERROR_NONE;
}
...
UINT32 OV8858QT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
