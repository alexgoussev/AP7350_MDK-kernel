...
#include "s5k5e2yaqtmipiraw_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K5E2YAQT_SENSOR_ID,
...
static kal_uint16 Agolden_R = 0x70,  Agolden_G = 0x9b, Agolden_B = 0x5e, Acurrent_R = 0, Acurrent_G = 0,  Acurrent_B = 0;


#ifdef S5K5e2ya_USE_AWB_OTP
static void s5k5e2ya_MIPI_read_otp_wb(void)
{
   	kal_uint16 current_RH = 0, current_RL = 0, current_R = 0, 
		       current_GH = 0, current_GL = 0, current_G = 0,  
		       current_BH = 0, current_BL = 0, current_B = 0;
		       
    uint16_t wb_flag = 0,retry=0;

                //read it before
               if(!( !Acurrent_G || !Acurrent_R || !Acurrent_B))
               	{
	        	   ...
	        	   write_cmos_sensor(0x0a02, 0x05); //page 5
				   write_cmos_sensor(0x0a00, 0x01);
Rtry_read_otp:			
	               mdelay(100);
				   wb_flag = read_cmos_sensor(0x0A0A);
	               if(0x1==wb_flag)//enable:0x01 disable:0x11
	               	{
	               	    module_id =  read_cmos_sensor(0x0A0B);
		        		current_RH = read_cmos_sensor(0x0A14);
						current_RL = read_cmos_sensor(0x0A15);
						current_GH = read_cmos_sensor(0x0A16);
						current_GL = read_cmos_sensor(0x0A17);
		        		current_BH = read_cmos_sensor(0x0A18);
						current_BL = read_cmos_sensor(0x0A19);
						current_R  = (current_RH<<8)|current_RL;
						current_G  = (current_GH<<8)|current_GL;
						current_B  = (current_BH<<8)|current_BL;
						printk("s5k5e2yaQT otp use group 1 ,mid = 0x%x\n ",module_id);
						printk("RH = 0x%x , RL = 0x%x , GH = 0x%x , GL = 0x%x , BH = 0x%x , BL = 0x%x\n",current_RH,current_RL,current_GH,current_GL,current_BH,current_BL);
	               	}
				   else 
				   	{ 
	                   wb_flag = read_cmos_sensor(0x0A1A);
	                   if(0x1==wb_flag)//enable:0x01 disable:0x11
					    	{
	                       		module_id =  read_cmos_sensor(0x0A1B);
				        		current_RH = read_cmos_sensor(0x0A24);
								current_RL = read_cmos_sensor(0x0A25);
								current_GH = read_cmos_sensor(0x0A26);
								current_GL = read_cmos_sensor(0x0A27);
				        		current_BH = read_cmos_sensor(0x0A28);
								current_BL = read_cmos_sensor(0x0A29);
								current_R  = (current_RH<<8)|current_RL;
								current_G  = (current_GH<<8)|current_GL;
								current_B  = (current_BH<<8)|current_BL;
								printk("s5k5e2yaQT otp use group 2 ,mid = 0x%x\n ",module_id);
								printk("RH = 0x%x , RL = 0x%x , GH = 0x%x , GL = 0x%x , BH = 0x%x , BL = 0x%x\n",current_RH,current_RL,current_GH,current_GL,current_BH,current_BL);
					    	}
						else
							{
								 wb_flag = read_cmos_sensor(0x0A2A);
	                             if(0x1==wb_flag)//enable:0x01 disable:0x11
									{
											module_id =  read_cmos_sensor(0x0A2B);
							        		current_RH = read_cmos_sensor(0x0A34);
											current_RL = read_cmos_sensor(0x0A35);
											current_GH = read_cmos_sensor(0x0A36);
											current_GL = read_cmos_sensor(0x0A37);
							        		current_BH = read_cmos_sensor(0x0A38);
											current_BL = read_cmos_sensor(0x0A39);
											current_R  = (current_RH<<8)|current_RL;
											current_G  = (current_GH<<8)|current_GL;
											current_B  = (current_BH<<8)|current_BL;
											printk("s5k5e2yaQT otp use group 3 ,mid = 0x%x\n ",module_id);
											printk("RH = 0x%x , RL = 0x%x , GH = 0x%x , GL = 0x%x , BH = 0x%x , BL = 0x%x\n",current_RH,current_RL,current_GH,current_GL,current_BH,current_BL);
									}
								else
									{//err
                   ...
                   if(!current_G ||!current_R || !current_B)
                   	{
                      retry++;
					  printk("s5k5e2yaQT otp retry ,retry = %d\n ",retry);
					  if(retry<5)
					  	{
					      goto Rtry_read_otp;
					  	}
  		...
  		printk("s5k5e2yaQT otp current_R=0x%x, current_G=0x%x, current_B=0x%x\n", current_R,current_G,current_B);
		write_cmos_sensor(0x0a00, 0x04); 
		write_cmos_sensor(0x0a00, 0x00);
		if(!current_G || !current_R || !current_B)
     		printk("WB update error!");
   		r_ratio = 512 * Agolden_R * current_G /( Agolden_G * current_R );
   		b_ratio = 512 * Agolden_B * current_G /( Agolden_G * current_B );
   		printk("s5k5e2yaQT otp  r_ratio=0x%x, b_ratio=0x%x\n", r_ratio, b_ratio);      
}
...
static void s5k5e2ya_MIPI_algorithm_otp_wb1(void)
{
	...
	else                      
   {
    ...
    printk("s5k5e2yaQT otp R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN, G_GAIN);    
}
...
CheckSubid:	
    ...
    if(module_id==OTP_MID_Qtech)
    	{
    	   printk("s5k5e2ya otp use OTP_MID_Qtech");
           return ERROR_NONE;
    	}
    else 
    	{
...
UINT32 S5K5E2YAQT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
