...
static struct otp_struct current_otp={0};
//R/G and B/G ratio of typical camera module is defined here
static kal_uint16 RG_Ratio_Typical = 0x11A; //RG_TYPICAL
static kal_uint16 BG_Ratio_Typical = 0x11B;//BG_TYPICAL
...
{ 
	...
	{ 
		...
		{
			addr = 0x7026; // base address of VCM Calibration group 2 
		}
	else if((otp_flag & 0x0c) == 0x04)
		{
			addr = 0x702b; // base address of VCM Calibration group 2 
		}
	if(addr != 0) {
			(*otp_ptr).flag = 0x40; 
			...
			(*otp_ptr).rg_ratio = 0; // not info and AWB in OTP 
			(*otp_ptr).bg_ratio = 0; 
			(*otp_ptr).light_rg = 0; 
			(*otp_ptr).light_bg = 0;
			printk("AWB is empty!\n");
	...
	{ 
		...
		{
			addr = 0x7037; // base address of VCM Calibration group 2 
		}
	...
	{ 
		addr = 0x70b7; // base address of Lenc Calibration group 2 
	} 
	if(addr != 0) 
	{
		(* otp_ptr).flag |= 0x10;
...
{ 
	...
	if ((*otp_ptr).flag & 0x40) {
		if((*otp_ptr).light_rg == 0){
			...
			rg = (*otp_ptr).rg_ratio * (current_otp.light_rg + 512) / 1024;
		}
		if((*otp_ptr).light_bg == 0){
			bg = (*otp_ptr).bg_ratio;
		}
		else{
			bg = (*otp_ptr).bg_ratio * (current_otp.light_bg + 512) / 1024;
		}		
		//calculate G gain
		...
		printk("AWB R_gain=0x%x B_gain=0x%x G_gain=0x%x\n",R_gain,B_gain,G_gain);
		if (R_gain>0x400) { 
