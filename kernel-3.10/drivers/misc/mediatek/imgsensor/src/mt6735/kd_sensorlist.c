...
struct platform_device image_sensor_device = {
    .name      = "image_sensor",
    .id                = -1,
};
...
{
	device_create_file(&(image_sensor_device.dev),&dev_attr_camera_name);
	device_create_file(&(image_sensor_device.dev),&dev_attr_main_camera_name);
	device_create_file(&(image_sensor_device.dev),&dev_attr_sub_camera_name);
	device_create_file(&(image_sensor_device.dev),&dev_attr_currSensorName);
...
#endif
    if(platform_device_register(&image_sensor_device)!=0) {
             
    PK_ERR("unable to register image_sensor device.\n");
    return -ENODEV;
    }
