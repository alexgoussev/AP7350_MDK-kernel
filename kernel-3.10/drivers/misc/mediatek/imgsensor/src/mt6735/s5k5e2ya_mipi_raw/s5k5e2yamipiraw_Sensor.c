...
Rtry_read_otp:			
	               ...
	               if((wb_flag&(1<<7))==0)//group 1   0: Empty/valid;
	               	{
	                 	printk("s5k5e2ya otp use group 1 ,mid = 0x%x\n ",wb_flag);
						module_id = wb_flag;
		        		current_R =  read_cmos_sensor(0x0A06);
						current_G =  read_cmos_sensor(0x0A07);
		        		current_B =  read_cmos_sensor(0x0A08); 
						
						golden_R =  read_cmos_sensor(0x0A09);
		        		golden_G =  read_cmos_sensor(0x0A0a);
						golden_B =  read_cmos_sensor(0x0A0B); 				  
	               	}
				   else 
				   	{ 
	                    wb_flag = read_cmos_sensor(0x0A0c);
					    if((wb_flag&(1<<7))==0)//group 2   0: Empty/valid;
					    	{
	                       		printk("s5k5e2ya otp use group 2 ,mid = 0x%x\n ",wb_flag);
							   	module_id = wb_flag;
				        		current_R =  read_cmos_sensor(0x0A0e);
								current_G =  read_cmos_sensor(0x0A0f);
				        		current_B =  read_cmos_sensor(0x0A10); 
								
								golden_R =  read_cmos_sensor(0x0A11);
				        		golden_G =  read_cmos_sensor(0x0A12);
								golden_B =  read_cmos_sensor(0x0A13); 	
					    	}
						else
							{
								wb_flag = read_cmos_sensor(0x0A14);
								if((wb_flag&(1<<7))==0)//group 3   0: Empty/valid;
									{
										printk("s5k5e2ya otp use group 3 ,mid = 0x%x\n ",wb_flag);
										module_id = wb_flag;
										current_R =  read_cmos_sensor(0x0A16);
										current_G =  read_cmos_sensor(0x0A17);
										current_B =  read_cmos_sensor(0x0A18); 
										
										golden_R =	read_cmos_sensor(0x0A19);
										golden_G =  read_cmos_sensor(0x0A1a);
										golden_B =	read_cmos_sensor(0x0A1B);	
									}
								else
									{//err
										golden_R = 0; 
