...
#define S5K3H7YX_USE_OTP 1
#ifdef S5K3H7YX_USE_OTP
#define GAIN_DEFAULT       	     0x0100
#define OTP_SIZE 452
u8 OTPData[OTP_SIZE];

extern  int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata);
extern bool read_3H7_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);

static kal_uint16 otp_flag = 0;
...
static void write_epprom_reg(kal_uint16 addr, kal_uint16 para)
{
...
static kal_uint16 S5K3h7yx_read_otp_vendor_id(void)
{
      uint16_t ModuleHouseID = 0, 
               info_flag = 0;
      
      info_flag = read_epprom_reg(0x0000);
      otp_flag = info_flag;
      //printk("%s,info_flag = 0x%x\n\n",__func__,otp_flag);
      ModuleHouseID = read_epprom_reg(0x0001);
      //printk(" ModuleHouseID = 0x%x\n", ModuleHouseID);
      return ModuleHouseID;
}

static void s5k3h7yx_update_awb(void)
{
   	kal_uint16 R_Gr_H = 0, R_Gr_L = 0, B_Gr_H = 0, 
		       B_Gr_L = 0, Gb_Gr_H = 0,  Gb_Gr_L = 0,
		       R_Gr = 0, B_Gr = 0, Gb_Gr = 0;
   	kal_uint16 g_R_Gr_H = 0, g_R_Gr_L = 0, g_B_Gr_H = 0, 
		       g_B_Gr_L = 0, g_Gb_Gr_H = 0,  g_Gb_Gr_L = 0,
		       g_R_Gr = 0, g_B_Gr = 0, g_Gb_Gr = 0;
	...
	wb_flag = read_epprom_reg(0x0009);
	//printk("wb_flag = 0x%x\n", wb_flag);
	if(wb_flag == 0)
	{
		printk("AWB is invalid!\n");
	}
	else{
		R_Gr_H = read_epprom_reg(0x000A);
		R_Gr_L = read_epprom_reg(0x000B);
		B_Gr_H = read_epprom_reg(0x000C);
		B_Gr_L = read_epprom_reg(0x000D);
		Gb_Gr_H = read_epprom_reg(0x000E);
		Gb_Gr_L = read_epprom_reg(0x000F);

		g_R_Gr_H = read_epprom_reg(0x0010);
		g_R_Gr_L = read_epprom_reg(0x0011);
		g_B_Gr_H = read_epprom_reg(0x0012);
		g_B_Gr_L = read_epprom_reg(0x0013);
		g_Gb_Gr_H = read_epprom_reg(0x0014);
		g_Gb_Gr_L = read_epprom_reg(0x0015);


		//printk("R_Gr_H = %x  R_Gr_L = %x B_Gr_H = %x B_Gr_L = %x Gb_Gr_H = %x Gb_Gr_L = %x\n", R_Gr_H, R_Gr_L,B_Gr_H, B_Gr_L,Gb_Gr_H, Gb_Gr_L);

		R_Gr = ((R_Gr_H & 0xffff)<<8)|R_Gr_L;
		B_Gr = ((B_Gr_H & 0xffff)<<8)|B_Gr_L;
		Gb_Gr = ((Gb_Gr_H & 0xffff)<<8)|Gb_Gr_L;

		g_R_Gr = ((g_R_Gr_H & 0xffff)<<8)|g_R_Gr_L;
		g_B_Gr = ((g_B_Gr_H & 0xffff)<<8)|g_B_Gr_L;
		g_Gb_Gr = ((g_Gb_Gr_H & 0xffff)<<8)|g_Gb_Gr_L;

		//printk("R_Gr = 0x%x B_Gr = 0x%x Gb_Gr = 0x%x g_R_Gr = 0x%x g_B_Gr = 0x%x g_Gb_Gr = 0x%x",R_Gr,B_Gr,Gb_Gr,g_R_Gr,g_B_Gr,g_Gb_Gr);

		r_ratio = 512 * g_R_Gr / R_Gr;
		b_ratio = 512 * g_B_Gr / B_Gr;
		//printk("r_ratio = 0x%x b_ratio = 0x%x\n",r_ratio,b_ratio);
    	...
    	write_epprom_reg(0x020e, (G_GAIN >> 8)&0xFF); //g_gain
    	write_epprom_reg(0x020f, G_GAIN & 0xFF); //g_gain
    	write_epprom_reg(0x0210, (R_GAIN >> 8)&0xFF); //r_gain
    	write_epprom_reg(0x0211, R_GAIN & 0xFF); //r_gain
    	write_epprom_reg(0x0212, (B_GAIN >> 8)&0xFF); //b_gain
    	write_epprom_reg(0x0213, B_GAIN & 0xFF); //b_gain
    	write_epprom_reg(0x0214, (G_GAIN >> 8)&0xFF); //g_gain
    	write_epprom_reg(0x0215, G_GAIN & 0xFF); //g_gain
   ...
   // write_cmos_sensor(0x0211, R_GAIN & 0xFFFF); //r_gain
    ...
    //write_cmos_sensor(0x0215, G_GAIN & 0xFFFF); //g_gain
    	printk("s5k3h7yx otp R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN, G_GAIN); 
	}
}
#endif
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_3H7_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;	
