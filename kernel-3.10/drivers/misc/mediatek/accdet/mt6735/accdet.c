...
#if 1//!defined ACCDET_EINT && !defined ACCDET_EINT_IRQ
...
#define LONG_AS                  ((AS_KEY) | LONG_PRESS)
...
#define DW_KEY_HIGH_THR	 (520) //0.50v=500000uv
#define DW_KEY_THR		 (260) //0.20v=200000uv
#define UP_KEY_THR       (90) //0.09v=90000uv
#define MD_KEY_THR		 (0)

static int key_check(int b)
{
	ACCDET_DEBUG("luosen adc_data: %d v\n",b);//sen.luo
...
static void send_key_event(int keycode,int flag)
{
	switch (keycode)
		...
		case LONG_MD:
			if(0 != call_status)
			{
			input_report_key(kpd_accdet_dev, KEY_ENDCALL, flag);
			input_sync(kpd_accdet_dev);
			ACCDET_DEBUG("[accdet]KEY_ENDCALL %d\n",flag);
			}
			else
			{
...
static DRIVER_ATTR(accdet_call_state,      0666, NULL,         accdet_store_call_state);
