...
static int K_param[6][3] =  {9999,9999,9999,9999,9999,9999,
                9999,9999,9999,9999,9999,9999,
                9999,9999,9999,9999,9999,9999};

static int Wave_Max,Wave_Min;

static int KXTJ2_1009_Calibration(struct i2c_client *client, s16 data[KXTJ2_1009_AXES_NUM])
{
...
//
	...
	long temp_sum = 0;
	s16 Axes_Avged[3];   
//input data array, do some operation then output data array for final g-sensor data
    for (i=0;i<KXTJ2_1009_AXES_NUM;i++)
    {
		//assign raw array init value - need to check for different platform
		raw[i] = data[i];
    }
	rc=0;
	for (i=0;i<3;i++)
	{
	  ...
	  if ( abs(Axes_Avged[i]) < Offset_torlance) rc++;
	  else
	  ...
	  if (Wave_Max-Wave_Min > Stable_range ) rc+=100; //check over range
	}
	...
	for (i=0;i<KXTJ2_1009_AXES_NUM;i++)
	{
		data[i] = raw[i]; //Output final data to data array
	}
...
static int KXTJ2_1009_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		//Added  by Knx - Call auto-calibration function, need to double confirm the call timing
		if(0 != (res = KXTJ2_1009_Calibration(client, obj->data)))
		{        
			GSE_ERR("Calling KXTJ2_1009_Calibration - I2C error: ret value=%d", res);
			return -3;
		}
...
// xiangfei added for sensor direction test
/*----------------------------------------------------------------------------*/
