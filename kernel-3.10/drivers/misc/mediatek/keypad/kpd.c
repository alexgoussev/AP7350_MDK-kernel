...
extern unsigned char g_cur_hall_light_level;/*0,the LCD is extinguished;1,the LCD is lightened*/
unsigned char g_hall_flag = 0;	/*0,open mobile phone cover;1,close mobile phone cover*/
...
/*----------------------------------------------------------------------------*/
static ssize_t hall_log_read(char *buf, char **start, off_t offset, int count, int *eof, void *data){
	buf[0] = kpd_hall_enable;
	return 1;
}

static ssize_t hall_log_write(struct file *filp, const char __user *buf, size_t count, loff_t *data)
{	
	char cmd_buf[256];
	int ret;
	...
	if(ret < 0)
		return -1;
	kpd_hall_enable = cmd_buf[0];
	
	return count;
}

static const struct file_operations hall_proc_fops = {
	.write = hall_log_write,
	.read = hall_log_read,		
};

void hall_proc_init(void){
	/*ÉèÖÃhall¿ª¹ØÔÚ/proc/ÏÂµÄ¿ØÖÆ½Úµã*/
	proc_create("enable_hall", 0666, NULL, &hall_proc_fops);
}
/********************************************************************************************/
/************************************************************************************************************************************************/
/* for autotest */
...
//if(kpd_hall_enable != '0')
{
		if(0==slid)
		{
			if(1 == g_cur_hall_light_level)/*avoid the LCD is lightened when the magnet is close to the hall component*/
			{
				g_hall_flag = 1;
		...
		else if(1==slid)//if((cur_hall_eint_state == EINT_PIN_FARAWAY )&&(0 != cur_hall_light_level))
		{
			...
			{
				g_hall_flag = 0;
				input_report_key(kpd_input_dev, KEY_HALL_OPEN, 1);
				input_sync(kpd_input_dev);
				udelay(10);
				input_report_key(kpd_input_dev, KEY_HALL_OPEN, 0);
				input_sync(kpd_input_dev);
			}
		}
}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	__set_bit(KEY_HALL_OPEN, kpd_input_dev->keybit);
