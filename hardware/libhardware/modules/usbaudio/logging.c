...
void log_pcm_config(struct pcm_config * config, const char* label) {
    ALOGD("log_pcm_config() - %s", label);
    ALOGD("  channels:%d", config->channels);
    ALOGD("  rate:%d", config->rate);
    ALOGD("  period_size:%d", config->period_size);
    ALOGD("  period_count:%d", config->period_count);
