...
{
    ALOGD("usb:audio_hw::device_get_parameters() keys:%s", keys);
    ...
    ALOGD("usb:audio_hw::device_get_parameters = %s", result_str);
...
{
    ALOGD("usb:audio_hw::out out_set_parameters() keys:%s", kvpairs);
    ...
    if (card >= 0 && device >= 0 && !profile_is_cached_for(out->profile, card, device)) {
        ...
        if (!out->standby)
            ...
            ALOGD("usb:audio_hw::out out_set_parameters() profile_read_device_info, saved_card=%d,saved_device=%d", saved_card,saved_device);
...
static char * out_get_parameters(const struct audio_stream *stream, const char *keys)
{
    ...
    ALOGD("usb:audio_hw::out out_get_parameters() keys:%s", keys);
    pthread_mutex_lock(&out->dev->lock);
    pthread_mutex_lock(&out->lock);
...
{
    ALOGD("usb:audio_hw::out adev_open_output_stream() handle:0x%X, device:0x%X, flags:0x%X",
...
{
    ALOGD("usb: audio_hw::in in_set_parameters() keys:%s", kvpairs);
...
{
    ALOGD("usb:audio_hw::start_input_stream(card:%d device:%d)",
...
static ssize_t in_read(struct audio_stream_in *stream, void* buffer, size_t bytes)
{
    ...
    if (in->standby) {
        ...
        if (start_input_stream(in) != 0) {
            pthread_mutex_unlock(&in->dev->lock);
            ALOGE(USB_TAG"in_read, start_input_stream fail");
            goto err;
        }
        in->standby = false;
    }
...
err:
    ...
    ALOGV("audio_hw:usb in_read, bytes =%d, num_read_buff_bytes=%d", bytes,num_read_buff_bytes);
    //Dump debug data
                                  ...
                                  audio_source_t source __unused)
{
    ...
    ALOGD("audio_hw:usb adev_open_input_stream, proxy_config.channels=%d", proxy_config.channels);
    proxy_prepare(&in->proxy, in->profile, &proxy_config);

    ALOGD("audio_hw:usb adev_open_input_stream, in->proxy.channels=%d", in->proxy.alsa_config.channels);
    in->standby = true;
...
{
    ALOGD("audio_hw:usb adev_set_parameters(%s)", kvpairs);
    ...
    if (param_val >= 0) {
        ...
        ALOGD("audio_hw:usb adev_set_parameters alsa_card(%d), alsa_device(%d)", alsa_card,alsa_device);
