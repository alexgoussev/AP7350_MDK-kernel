...
{
    ALOGD("profile_calc_min_period_size(%p, rate:%d)", profile, sample_rate);
    if (profile == NULL) {
        return DEFAULT_PERIOD_SIZE;
    } else {
        unsigned num_sample_frames = (sample_rate * BUFF_DURATION_MS) / 1000;
        ALOGD("profile_calc_min_period_size num_sample_frames=%d, profile->min_period_size=%d)", num_sample_frames, profile->min_period_size);
...
unsigned int profile_get_period_size(alsa_device_profile* profile, unsigned sample_rate)
{
    ...
    ALOGD("profile_get_period_size(rate:%d) = %d", sample_rate, period_size);
    return period_size;
}
...
{
    ALOGD("usb:audio_hw - read_alsa_device_config(c:%d d:%d t:0x%X)",
    ...
    if (alsa_hw_params == NULL) {
        ALOGD("read_alsa_device_config no alsa_hw_params get, return");
        return -EINVAL;
    }
    ...
    ALOGD("read_alsa_device_config finish");
    return ret;
}
...
bool profile_read_device_info(alsa_device_profile* profile)
{
    ...
    ALOGD("default_config chans:%d rate:%d format:%d count:%d size:%d",
    ...
    struct pcm_params * alsa_hw_params = pcm_params_get(profile->card,
                                                        ...
                                                        profile->direction);
    if (alsa_hw_params == NULL) {
        ALOGD("profile_read_device_info alsa_hw_params NULL, return");
        return false;
    }
    ...
    ALOGD("profile_read_device_info done");
    return true;
}
