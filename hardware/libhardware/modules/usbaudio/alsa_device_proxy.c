...
#ifdef LOG_PCM_PARAMS
    log_pcm_config(&proxy->alsa_config, "proxy_prepare finish()");
#endif
...
#ifdef LOG_PCM_PARAMS
        log_pcm_config(&proxy->alsa_config, "proxy_open alsa config");
#endif
...
#ifdef LOG_PCM_PARAMS
        log_pcm_config(&proxy->alsa_config, "proxy_open fail alsa config");
#endif
        pcm_close(proxy->pcm);
        proxy->pcm = NULL;
        return -ENOMEM;
