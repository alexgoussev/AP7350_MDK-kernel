...
function getoutputdir()
{
	...
	then
		...
		pwdnodir=$(echo $T |sed -e "s/.*\/\(.*\)/\1/")
		target=$OUT_DIR_COMMON_BASE/$pwdnodir/
	fi
	echo $target
}
...
function _mk()
{
	...
	local tee=true
	local newlunch=
	#采用另外一种方庿andorid)进行模块编译
	local another=false
	local OPTIND
	while getopts ":tna" opt
	do
		case $opt in
		t)
			tee=false
...
#     		;;
	...
	local selection=
	if [ -e makeMtk.ini ] && [ "$newlunch" != "true" ]
	then
		while read selection
		do
			break
		done < makeMtk.ini
		if [ "${TARGET_PRODUCT}-${TARGET_BUILD_VARIANT}" = "$selection" ]
		then
			echo 跳过lunch
		else
			lunch $selection
...
#	echo tee=$tee;
	...
	clean|c)
		...
		;;
	*)
		echo "错误:请设置操作模庿n|r|c)"
		return
	esac
	shift   #去第一个参擿	
#	echo  new=$new remake=$remake clean=$clean
	
	if [ "$new" = "true" ] || [ "$clean" = "true" ]
	then
		local variant=$(echo -n $@ | sed -e "s/\([^ ]*\)/clean-\1/g")
		
		if [ -n "$variant" ]
		then
			case ${variant} in
			clean-k)
				echo 开始清理clean-kernel
				make clean-kernel
				;;
			*)
				echo 开始清理{variant}
	...
	local cpunum=$(getcpunum)
	local pro=$(getproject)
	local debug=
	if [ $(getbuildmode) = "eng" ]
	then
		debug="_debug"
	...
	then
		export newall=$new
		local teelogfile=$(getTeeFile);
		if [ "$tee" = "true" ]
		then
			make -j$cpunum $@ 2>&1 |tee $teelogfile
		else
			make -j$cpunum $@ 2>&1
		fi
	else
		...
		do
			case $1 in
			pl)
				makepl $another $pro -j$cpunum  $outtee
				;;
			lk)
			    makelk $another $pro -j$cpunum  $outtee
			    ;;
			kernel|k)
				makekernel $another ${pro}${debug} -j$cpunum  $outtee
				;;
			menu)
				makemenu $another ${pro}${debug} -j$cpunum  $outtee
				;;
			*)
				echo 错误的模快				return
				;;	
...
function mk()
{
	...
	#用于解决$@中有 -n之类的东裿	local DASH_ARGS=$(echo " " "$@" | awk -v RS=" " -v ORS=" " '/^-.*$/')
	local targ=$(echo "$@" | awk -v RS=" " -v ORS=" " '/^[^-].*$/')
	_mk $DASH_ARGS $targ
}

#$1=full_ckt6582_w_l-eng
#CKT6582_W_L
function getproject()
{
#	echo -n $1 | sed -e "s/[^_]*_\(.*\)-.*$/\1/"
...
function makepl()
{
	...
	then
		(\cd $T ;\
		echo 用andorid 原始方法进行编译 ;\
		if [ "$tee" = "true" ]
		then
			make $3 pl 2>&1 |tee $teelogfile
		else
			make $3 pl 2>&1
		fi
		)
	else
	    (\cd $T/bootable/bootloader/preloader ;\
	    echo 用简洁方法编译;\
	    if [ "$tee" = "true" ]
		then
			TARGET_PRODUCT=$2 ./build.sh 2>&1 |tee $teelogfile
		else
			TARGET_PRODUCT=$2 ./build.sh 2>&1
		fi
	    echo 复制文件到输出目录;\
	    cp -v $outputdir/target/product/$2/obj/PRELOADER_OBJ/bin/preloader_$2.bin $outputdir/target/product/$2/ ;\
	    cd - >/dev/null)
	fi
}
...
function makelk()
{
	...
	then
		...
		if [ "$tee" = "true" ]
		then
			make $3 lk 2>&1 |tee $teelogfile
		else
			make $3 lk 2>&1
		fi
		)
	else
	    (\cd $T/bootable/bootloader/lk ;\
	    if [ "$tee" = "true" ]
		then
			 make $3 $2 2>&1 |tee $teelogfile
		else
			 make $3 $2 2>&1
		fi
	    echo 复制文件到输出目录;\
	    cp -v ./build-$2/lk.bin $outputdir/target/product/$2/ ;\
	    cp -v ./build-$2/logo.bin $outputdir/target/product/$2/ ;\
	    cd - >/dev/null)
    fi 
}


#$another ${pro}${debug} -j$cpunum  $tee
#   1      2               3          4 
function makekernel()
{
	T=$(gettop)
	...
	then
		...
		if [ "$tee" = "true" ]
		then
			 make $3 kernel bootimage 2>&1 |tee $teelogfile
		else
			 make $3 kernel bootimage 2>&1
		fi
		)
	else
	    (\cd $T ;\
	    if [ "$tee" = "true" ]
		then
			mmm kernel-3.10/:$(get_build_var PRODUCT_OUT)/kernel 2>&1 |tee $teelogfile
			if [  ${PIPESTATUS[0]} -eq 0 ]
			then
				make $3 ramdisk-nodeps 2>&1 
				make $3 bootimage-nodeps 2>&1
			fi
		else
			mmm kernel-3.10/:$(get_build_var PRODUCT_OUT)/kernel 2>&1
			if [  ${PIPESTATUS[0]} -eq 0 ]
			then
				make $3 ramdisk-nodeps 2>&1 
				make $3 bootimage-nodeps 2>&1
...
#$another ${pro}_${debug} -j$cpunum  $tee
#   1      2                3          4
#  没有璿
function makemenu()
{
    ...
    local ARCH=$(get_build_var TARGET_ARCH)
    local def=$(get_build_var KERNEL_DEFCONFIG)
    (\cd $T/kernel-3.10 ;\
    mkdir out >/dev/null 2>&1 ;\
    make O=out TARGET_ARCH=${ARCH} ${def} ;\
    make O=out TARGET_ARCH=${ARCH} menuconfig;\
    echo 复制文件到输出目录;\
    cp -v ./out/.config ./arch/${ARCH}/configs/${def};\
    cd - >/dev/null)
}
...
{
cat <<EOF
用法:代码直接加入丿j cpunum,原始的命令做了保瓿依然可以直接使用
	mk [option] (n|r|c) [module]
	
	option
	   -n   new,是否重新加载lunch,如果不是则使用上次的设置,如果上次没有设置,则依然加载lunch
	   -t   tee,是否tee输出,tee目前直接设置到了根项目下的build.log,-t是不输出
	   -a   another,是否采用另外一种方法进行编譿	                另外一种方法通常是简化的方法,会编译比较快,对kernel的编譿目前还有点问頿
	                注意编译了kernel还要更新bootimage才可以下軿	                默认的方汿就是android的模块编译方汿	n      对项目进行new,会先进行clean
	r      对项目进行remake
	c      对项目进行clean
	module 要处理的模块pl,lk,kernel|k,menu,如果为空,是全部模囿可以一次写入多个模囿	       如果是systemimage之类的模囿请用原始的make进行编译
...
例子
    ...
    使用原始方法编译
    make
    编译pl,巿䶿把log输出到build.log
    mk -t r pl
    编译pl,并把log输出到build.log
    mk r pl
    采用另外一种方法编译lk
    mk -a r lk
    编译kernel
...
历史
   1.00 
   1.1 默认输出log,-t为不输出build.log
       修改c 模式䶿依然还会进行编译的问頿       编译kernel的时便直接编译了bootimage
